...
import HachiBot.modules.sql.kuki_sql as sql
    ...
    if match:
        ...
        if is_kuki:
            ...
            update.effective_message.edit_text(
                "HachiBot AI disable by {}.".format(mention_html(user.id, user.first_name)),
...
def kukiadd(update: Update, context: CallbackContext) -> str:
    ...
    if match:
        ...
        if is_kuki:
            ...
            update.effective_message.edit_text(
                "HachiBot AI enable by {}.".format(mention_html(user.id, user.first_name)),
                parse_mode=ParseMode.HTML,
            )
...
def kuki(update: Update, context: CallbackContext):
    ...
    msg = "Choose an option to activated HachiBot ai"
...
def kuki_message(context: CallbackContext, message):
    ...
    if message.text.lower() == "HachiBot":
        return True
    if reply_message:
